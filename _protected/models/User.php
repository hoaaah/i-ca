...
class User extends UserIdentity
{
    ...
    public function rules()
    {
        return [
            ...
            [['unit_id', 'sub_unit_id', 'kd_user'], 'integer'],
            ['status', 'required'],
    ...
    {
        $unit = RefUnit::find();
        $unit = $unit->orderBy('id')->all();
        $unitArray = ArrayHelper::map($unit,'id','nama_unit');
        return $unitArray;
    }

    public function getSubUnitArray()
    {
        $unit = RefSubUnit::find()->select(["sub_unit_id", "CONCAT(unit_id, '.', sub_unit_id, ' ', nama_sub_unit) AS nama_sub_unit"]);
        if($this->unit_id) $unit->where(['unit_id' => $this->unit_id]);
        $unit = $unit->orderBy('unit_id, sub_unit_id')->asArray()->all();
        $unitArray = ArrayHelper::map($unit, 'sub_unit_id','nama_sub_unit');
        return $unitArray;
    }

    public function getKdUserArray()
    {
        $refUser = RefUser::find()->all();
        $refUserArray = ArrayHelper::map($refUser, 'id','name');
        return $refUserArray;
